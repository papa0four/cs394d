...
class LogGrader(Grader):
    ...
    def test_train(self):
        ...
        for step in range(200):
            ...
            assert step in loss, "No train_loss found for epoch=%d, iteration=%d" % (step // 20, step % 20)
            got = loss[step]
            assert abs(got - expect) < 1e-2, "train_loss epoch=%d, iteration=%d expected %f got %f" % (
                step // 20,
                step % 20,
                expect,
                got,
        ...
        for epoch in range(10):
            ...
            assert 20 * epoch + 19 in acc or 20 * epoch + 20 in acc, (
                "No train_accuracy logging found for epoch %d" % epoch
            )
            got = acc[20 * epoch + 19] if 20 * epoch + 19 in acc else acc[20 * epoch + 20]
            assert abs(got - expect) < 1e-2, "train_accuracy epoch=%d expected %f got %f" % (epoch, expect, got)
    ...
    def test_val(self):
        ...
        for epoch in range(10):
            ...
            assert 20 * epoch + 19 in acc or 20 * epoch + 20 in acc, (
                "No val_accuracy logging found for epoch %d" % epoch
            )
            got = acc[20 * epoch + 19] if 20 * epoch + 19 in acc else acc[20 * epoch + 20]
            assert abs(got - expect) < 1e-2, "val_accuracy epoch=%d expected %f got %f" % (epoch, expect, got)
